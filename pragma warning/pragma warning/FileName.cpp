...
#include<stdio.h>
#include<string.h>
// #include<stdlib.h> // long 및 NULL 사용을 위해 추가 (사용자 요청에 따라 제거)

// 무슨일을 하는 함수인지 실행해보고 주석작성
void clear_input_buffer(void)
{
	// [함수 역할]: 이 함수는 키보드 입력 후 입력 버퍼에 남아있는 찌꺼기 문자(주로 '\n')를
	// 모두 읽어 들여 버립니다. 다음 입력 함수(예: fgets)가 정상적으로 작동하도록 버퍼를 정리합니다.
	int c;
	while ((c = getchar()) != '\n' && c != EOF)
	{
		;
...
int main(void)
{
	...
	// 포인터 변수에 각 배열의 첫 번째 주소를 할당합니다. (배열 이름은 배열의 시작 주소입니다.)
	char* p_str1 = str1;
	char* p_str2 = str2;
	char* p_copy = copy;
	...
	// [주의] scanf_s 사용 시 배열 크기 인수를 추가해야 합니다.
	...
	// puts 함수를 사용하여 문자열을 출력하고 자동으로 개행(\n)합니다.
	puts(p_str1);
	puts(p_str2);
	...
	// 무슨일을 하는 부분인지 작성
	printf("\n---strlen practice---\n");
	// [기능]: strlen 함수를 사용하여 각 문자열(str1, str2)의 **널 문자(\0)를 제외한** 길이를 계산하고 출력합니다.
	...
	// 입력 버퍼에 남겨둡니다. 바로 다음에 호출되는 fgets가 이 잔여 '\n'을 읽어 입력을 건너뛰는 것을 방지하기 위해 버퍼를 비웁니다.

	printf("\nEnter one sentence(can include spaces):");
	// fgets를 사용하여 공백을 포함하는 문장 전체를 입력받습니다.
	...
	// [기능]: strcpy 함수를 사용하여 p_str1이 가리키는 문자열 전체를 p_copy 배열로 복사합니다.
	...
	// [기능]: strcat을 수행하기 전에, str1에 str2를 붙였을 때 str1의 버퍼(100바이트)를 초과하지 않는지 확인합니다.
	if (strlen(p_str1) + strlen(p_str2) < sizeof(str1)) {
		// strcat 함수를 사용하여 p_str2의 문자열을 p_str1의 끝에 이어 붙입니다. (p_str1의 내용이 변경됩니다.)
	...
	// [기능]: strcmp 함수를 사용하여 현재 str1과 str2를 사전 순서(lexicographical order)로 비교합니다.
	{
	...
	// ' %c'는 남아있는 공백 문자나 개행 문자를 무시하고 실제 문자 하나를 입력받습니다.
	scanf(" %c", &ch);

	// [기능]: strchr 함수를 사용하여 str1 문자열 내에서 문자 ch가 처음 나타나는 위치의 포인터를 찾습니다.
	pos = strchr(p_str1, ch);

	if (pos != NULL) {
		// 포인터 뺄셈 (pos - p_str1)을 통해 str1의 시작 주소로부터 문자가 위치한 인덱스(거리)를 계산합니다.
	...
	// [기능]: strstr 함수를 사용하여 sentence 문자열 내에서 str2 문자열(부분 문자열)이 처음 나타나는 위치의 포인터를 찾습니다.
	pos = strstr(p_sentence, p_str2);
	if (pos != NULL) {
		printf("Found\"%s\"inside sentence.\n", p_str2);
		// 찾은 위치(pos)부터 문장의 끝까지 나머지 문자열을 출력합니다.
	...
	//왜 이 실습에서는 temp에 문자열을 복사하는가?
		...
		// 때문에 원본을 보존하기 위해 p_temp에 복사본을 만들어 분리 작업을 수행합니다.
	strcpy(p_temp, p_sentence);

	// [기능]: strtok 함수를 사용하여 p_temp를 공백(" ")을 기준으로 토큰(단어)으로 분리합니다.
	char* token = strtok(p_temp, " ");
	int i = 1;
	while (token != NULL) {
		printf("Token%d:", i++);
		puts(token);
		// [수정] 다음 토큰을 얻을 때는 NULL을 첫 번째 인수로 사용하고, 구분자도 " "로 지정해야 합니다.
		token = strtok(NULL, " ");
	}
	//------------------------------------------------------
	printf("\nProgram finished.\n");
